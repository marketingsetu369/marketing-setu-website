import { TranslationDictionary } from "./types";

export const hi: TranslationDictionary = {
  // Navigation
  nav_home: "मुख्यपृष्ठ",
  nav_services: "सेवाएं",
  nav_pricing: "कीमतें",
  nav_about: "हमारे बारे में",
  nav_reviews: "समीक्षाएं",
  nav_blog: "ब्लॉग",
  nav_contact: "संपर्क",
  nav_whatsapp_btn: "व्हाट्सएप करें",
  nav_explore_services: "सेवाएं खोजें",

  // Footer & Contact
  footer_tagline: "आपके व्यवसाय और आपके ग्राहकों के बीच का सेतु — भारत के छोटे व्यवसायों के लिए व्हाट्सएप मार्केटिंग, लैंडिंग पेज और लोकल ग्रोथ टूल्स।",
  footer_company: "कंपनी",
  footer_services: "सेवाएं",
  footer_get_in_touch: "संपर्क करें",
  footer_rights: "© 2026 मार्केटिंगसेतु। सर्वाधिकार सुरक्षित।",
  footer_privacy: "गोपनीयता नीति",
  footer_terms: "सेवा की शर्तें",

  // General buttons / shared
  btn_whatsapp_chat: "💬 व्हाट्सएप पर चैट करें",
  btn_free_consultation: "मुफ़्त सलाह",
  btn_ask_whatsapp: "💬 व्हाट्सएप पर पूछें",
  btn_get_started: "शुरू करें",
  btn_view_plan: "प्लान देखें",
  btn_see_all_services: "सभी सेवाएं देखें →",

  // Home Page
  home_hero_badge_platform: "#1 प्लेटफॉर्म",
  home_hero_badge_software: "डिजिटल मार्केटिंग और स्मार्ट कार्ड सॉफ्टवेयर",
  home_hero_eyebrow: "सेतु · प्रगति का मार्ग",
  home_hero_title_1: "ऑल-इन-वन डिजिटल मार्केटिंग प्लेटफॉर्म",
  home_hero_title_for: "छोटे व्यवसायों के लिए",
  home_hero_title_gradient: "सिर्फ एक मैसेज से।",
  home_hero_lead: "अपनी डिजिटल पहचान बनाएं, ग्राहकों से सीधे जुड़ें, अपनी मार्केटिंग मैनेज करें और अपना व्यवसाय बढ़ाएं — सब कुछ एक ही आसान प्लेटफॉर्म से।",
  home_hero_cta_create_card: "डिजिटल बिजनेस कार्ड बनाएं",
  home_hero_stat_1: "500+ जुड़े हुए व्यवसाय",
  home_hero_stat_2: "98% ग्राहक संतुष्टि",
  home_hero_stat_3: "3 गुना औसत वृद्धि",
  home_hero_stat_4: "24/7 व्हाट्सएप सपोर्ट",

  home_services_eyebrow: "हमारी सेवाएं",
  home_services_heading: "आपके व्यवसाय की पहचान बढ़ाने के लिए सभी आवश्यक टूल्स",
  home_services_subtext: "अपने ग्राहकों तक आसानी से पहुंचने के तरीके: व्हाट्सएप, गूगल और सीधे फोन कॉल के माध्यम से।",

  home_pricing_eyebrow: "कीमत और प्लान्स",
  home_pricing_heading: "आप चाहे कहीं से भी शुरुआत कर रहे हों, आपके लिए सही प्लान मौजूद है",
  home_pricing_subtext: "अपने व्यवसाय के लिए सही डिजिटल ग्रोथ प्लान चुनें — शून्य सेटअप शुल्क के साथ कभी भी अपग्रेड या बदलें।",

  home_why_eyebrow: "मार्केटिंगसेतु क्यों चुनें",
  home_why_heading: "परिणाम-उन्मुख। विश्वसनीय। किफायती।",

  home_how_eyebrow: "यह कैसे काम करता है",
  home_how_heading: "3 आसान स्टेप्स में शुरू करें",

  home_testimonials_eyebrow: "ग्राहकों के अनुभव",
  home_testimonials_heading: "हमारे ग्राहक क्या कहते हैं",
  home_testimonials_subtext: "मार्केटिंगसेतु की मदद से आगे बढ़ रहे पूरे महाराष्ट्र के छोटे व्यवसायों की सच्ची कहानियां।",

  home_cta_heading: "अधिक ग्राहकों तक पहुंचने के लिए तैयार हैं?",
  home_cta_description: "हमें व्हाट्सएप पर मैसेज करें और मुफ़्त सलाह पाएं — अधिकांश व्यवसायों को एक घंटे के भीतर जवाब मिलता है।",

  // About Page
  about_hero_breadcrumb: "हमारे बारे में",
  about_hero_eyebrow: "मार्केटिंगसेतु के बारे में",
  about_hero_title_pre: "हम सेतु — ",
  about_hero_title_highlight: "पुल",
  about_hero_title_post: " — बनाते हैं, छोटे व्यवसायों और ग्राहकों के बीच",
  about_hero_lead: "मार्केटिंगसेतु की शुरुआत एक सरल विश्वास के साथ हुई थी: हर स्थानीय दुकान, क्लिनिक और छोटे व्यवसाय को बड़े ब्रांड्स जैसी ही डिजिटल पहुंच मिलनी चाहिए, वह भी बड़े बजट के बिना।",
  about_story_eyebrow: "हमारी कहानी",
  about_story_heading: "मिस्ड कॉल से लेकर खोए हुए ग्राहकों तक — हमने इस अंतर को करीब से देखा",
  about_story_p1: "Working closely with small businesses across Maharashtra, our team kept hearing the same story: a customer calls, no one picks up in time, and the lead is gone. Meanwhile, WhatsApp — the app already open on every customer's phone — was going completely unused for marketing.",
  about_story_p2: "मार्केटिंगसेतु इसी अंतर को पाटने के लिए बनाया गया है। हम व्हाट्सएप ऑटोमेशन, मिस्ड-कॉल रिप्लाई और फास्ट लैंडिंग पेजेस को एक किफायती वार्षिक प्लान में शामिल करते हैं।",
  about_serve_eyebrow: "हम किसे सेवा देते हैं",
  about_serve_heading: "हर प्रकार का छोटा व्यवसायी, सिर्फ एक व्हाट्सएप मैसेज दूर",
  about_serve_p1: "स्थानीय लॉन्ड्री से लेकर बुटीक, रेस्टोरेंट और रियल एस्टेट एजेंट्स तक — हमारे ग्राहक भारत की अर्थव्यवस्था की रीढ़ हैं। यदि आपके ग्राहक फोन का उपयोग करते हैं, तो वे सबसे पहले आप तक पहुंचेंगे।",
  about_mission_title: "हमारा उद्देश्य",
  about_mission_desc: "भारतीय छोटे व्यवसायों और ग्राहकों को जोड़ने वाला सबसे सरल और किफायती सेतु (पुल) बनना।",
  about_promise_title: "हमारा वादा",
  about_promise_desc: "कोई जटिल तकनीकी भाषा नहीं, कोई लंबे कॉन्ट्रैक्ट नहीं — केवल वो टूल्स जो सच में काम आएं।",
  about_values_eyebrow: "हमारे मूल्य",
  about_values_heading: "हमारे मूल्य",
  about_process_eyebrow: "हमारी प्रक्रिया",
  about_process_heading: "हम कैसे काम करते हैं",
  about_cta_heading: "क्या हम आपके व्यवसाय के लिए सही हैं?",
  about_cta_description: "व्हाट्सएप पर अपने व्यवसाय के बारे में बताएं — कोई दबाव नहीं, केवल सही और ईमानदार सलाह।",

  // Services Page
  services_hero_breadcrumb: "सेवाएं",
  services_hero_eyebrow: "हमारी सेवाएं",
  services_hero_title: "भारतीय ग्राहकों की संवाद शैली के अनुसार बनाए गए मार्केटिंग टूल्स",
  services_hero_lead: "पहले मिस्ड कॉल रिप्लाई से लेकर व्हाट्सएप ब्रॉडकास्ट तक, हर सेवा ग्राहकों को आकर्षित करने के लिए बनाई गई है.",
  services_cta_heading: "कौन सी सेवा आपके व्यवसाय के लिए सही है, इस पर उलझन है?",
  services_cta_description: "हमें बताएं कि आप क्या बेचते हैं — हम व्हाट्सएप चैट पर ही सही विकल्प सुझाएंगे।",

  // Pricing Page
  pricing_hero_breadcrumb: "कीमतें",
  pricing_hero_eyebrow: "कीमत और प्लान्स",
  pricing_hero_title: "आसान प्लान्स। बेहतरीन परिणाम। कोई छुपा हुआ शुल्क नहीं।",
  pricing_hero_lead: "हर प्लान का साल में एक ही बार भुगतान करना होता है। आज ही आसान प्लान से शुरुआत करें और जरूरत पड़ने पर अपग्रेड करें।",
  pricing_help_note: "सभी प्लान्स में ऑनबोर्डिंग सेटअप सपोर्ट शामिल है। प्लान चुनने में मदद चाहिए?",
  pricing_help_btn: "हमसे बात करें",
  pricing_faq_eyebrow: "प्रश्नोत्तरी",
  pricing_faq_heading: "अक्सर पूछे जाने वाले प्रश्न",
  pricing_cta_heading: "अभी भी सोच रहे हैं?",
  pricing_cta_description: "अपने व्यवसाय का प्रकार और शहर का नाम मैसेज करें — हम तुरंत सही प्लान सुझाएंगे।",

  // Compare Table
  compare_table_eyebrow: "प्लान्स की तुलना",
  compare_table_heading: "सभी खूबियां, आमने-सामने",
  compare_table_col_feature: "खूबी",

  // Reviews / Testimonials Page
  reviews_hero_breadcrumb: "समीक्षाएं",
  reviews_hero_eyebrow: "ग्राहकों के अनुभव",
  reviews_hero_title: "असली व्यवसाय। असली परिणाम। असली प्रगति।",
  reviews_hero_lead: "देखें कि कैसे मार्केटिंगसेतु ने महाराष्ट्र के छोटे व्यवसायों को अधिक ग्राहकों से जुड़ने में मदद की।",
  reviews_join_eyebrow: "जुड़ें",
  reviews_join_heading: "आपका व्यवसाय हमारी अगली सफलता की कहानी हो सकता है",
  reviews_join_desc: "अपने व्यवसाय के बारे में बताएं और देखें कि हमारे प्लान आपके लिए कैसे काम कर सकते हैं।",

  // Blog Page
  blog_hero_breadcrumb: "ब्लॉग",
  blog_hero_eyebrow: "मार्केटिंगसेतु ब्लॉग",
  blog_hero_title: "छोटे व्यवसायों के लिए सरल और व्यावहारिक मार्केटिंग सलाह",
  blog_hero_lead: "व्हाट्सएप मार्केटिंग, गूगल मैप्स और ऑनलाइन व्यापार बढ़ाने के बारे में आसान गाइड।",
  blog_cta_heading: "मार्केटिंग टिप्स सीधे व्हाट्सएप पर पाना चाहते हैं?",
  blog_cta_description: "हमें मैसेज करें और हम आपके व्यवसाय से संबंधित टिप्स शेयर करेंगे।",

  // Contact Page
  contact_hero_breadcrumb: "संपर्क",
  contact_hero_eyebrow: "संपर्क करें",
  contact_hero_title: "आइए आपके व्यवसाय को और अधिक ग्राहकों से जोड़ें",
  contact_hero_lead: "हमारे प्लान्स, सेवाओं या व्हाट्सएप मार्केटिंग के बारे में कोई प्रश्न है? हमें मैसेज भेजें, हमारी टीम मदद करेगी.",
  contact_info_title: "सीधे संपर्क करें",
  contact_info_subtitle: "हमें आपके व्यवसाय के बारे में सुनना पसंद आएगा। व्हाट्सएप करें या संदेश भेजें.",
  contact_form_title: "संदेश भेजें",
  contact_form_name: "आपका नाम",
  contact_form_phone: "फ़ोन नंबर",
  contact_form_business: "व्यवसाय का नाम",
  contact_form_plan: "रुचि का प्लान",
  contact_form_message: "हम आपकी किस प्रकार सहायता कर सकते हैं?",
  contact_form_submit: "संदेश भेजें",
  contact_form_success: "धन्यवाद! हम जल्द ही आपसे संपर्क करेंगे।",
  contact_form_name_placeholder: "आपका नाम",
  contact_form_phone_placeholder: "+91 90000 00000",
  contact_form_business_placeholder: "आपके बिज़नेस का नाम",
  contact_form_plan_starter: "Starter – ₹४,९९९",
  contact_form_plan_growth: "Growth – ₹९,९९९",
  contact_form_plan_pro: "Pro – ₹१४,९९९",
  contact_form_submitting: "भेजा जा रहा है...",

  // Dynamic Service Titles & Leads
  service_wa_title: "व्हाट्सएप मार्केटिंग",
  service_wa_lead: "अपने ग्राहकों तक सीधे व्हाट्सएप पर पहुंचें, जहां वे पहले से अपना समय बिता रहे हैं।",
  service_lp_title: "लैंडिंग पेज डिज़ाईन",
  service_lp_lead: "अतिरिक्त कस्टमर्स और पूछताछ लाने वाला फ़ास्ट और मोबाइल-फ्रेंडली सिंगल लैंडिंग पेज।",
  service_mc_title: "मिस्ड कॉल ऑटो एसएमएस",
  service_mc_lead: "कॉल न उठने या व्यस्त रहने पर कस्टमर को तत्काल ऑटोमैटिक एसएमएस रिप्लाई सेंड करें।",
  service_gb_title: "गूगल बिजनेस सेटअप",
  service_gb_lead: "अपने बिजनेस प्रोफाइल को गूगल मैप्स के टॉप पर लाएं ताकि लोग आपको आसानी से खोजें।",
  service_ie_title: "आय और व्यय ट्रैकर",
  service_ie_lead: "अपने बिज़नेस की दैनिक आय और खर्चों को ट्रैक करें और आसानी से मुनाफा जानें।",
  service_pc_title: "डिजिटल उत्पाद कैटलॉग",
  service_pc_lead: "अपने सभी प्रोडक्ट्स और सर्विसेज को एक प्रीमियम डिजिटल कैटलॉग पेज पर प्रस्तुत करें।",
  service_badge_popular: "सबसे लोकप्रिय",

  // Dynamic Plan Details
  plan_quick_name: "स्मार्ट डिजिटल बिजनेस कार्ड",
  plan_quick_price: "₹3,499",
  plan_quick_desc: "छोटे और नए व्यवसायों के लिए",
  plan_smart_name: "कैटेगरी स्मार्ट कार्ड",
  plan_smart_price: "₹7,999",
  plan_smart_desc: "बढ़ते हुए स्थानीय आउटलेट्स के लिए",
  plan_power_name: "प्रो",
  plan_power_price: "₹11,999",
  plan_power_desc: "कम्पलीट ऑटोमेशन और लीड मैनेजमेंट",

  plan_feat_landing_page: "१ लैंडिंग पेज",
  plan_feat_auto_wa: "व्हाट्सएप ब्रॉडकास्ट",
  plan_feat_auto_sms: "मिस्ड कॉल ऑटो एसएमएस",
  plan_feat_google: "गूगल मैप्स सेटअप",
  plan_feat_support: "समर्पित सपोर्ट",
  plan_feat_desc_included: "प्लान की सभी सुविधाएं उपलब्ध हैं",
  plan_help_choose: "हम मदद करेंगे।",

  // Why Us Features
  why_us_f1_title: "फास्ट डिलीवरी",
  why_us_f1_desc: "कैंपेन और डिज़ाइन पर त्वरित डिलीवरी, ताकि आपका व्यवसाय कभी न रुके।",
  why_us_f2_title: "प्रमाणित परिणाम",
  why_us_f2_desc: "हमारे क्लाइंट्स को कुछ ही हफ़्तों में लीड्स और सेल्स में वास्तविक ग्रोथ दिखती है।",
  why_us_f3_title: "किफायती दाम",
  why_us_f3_desc: "छोटे और बढ़ते व्यवसायों के बजट में बिल्कुल फ़िट होने वाला डिजिटल मार्केटिंग।",
  why_us_f4_title: "समर्पित सपोर्ट",
  why_us_f4_desc: "एक पर्सनल मैनेजर जो आपके गोल्स और बिज़नेस को अच्छे से समझे।",

  // How Steps
  how_s1_title: "संपर्क करें",
  how_s1_desc: "व्हाट्सएप या फॉर्म द्वारा हमसे कनेक्ट होकर अपने बिजनेस की जानकारी दें।",
  how_s2_title: "प्लानिंग और सेटअप",
  how_s2_desc: "हमारी टीम सही स्ट्रेटेजी तैयार कर आपकी कैंपेन जल्द लॉन्च करती है।",
  how_s3_title: "ग्रोथ ट्रैक करें",
  how_s3_desc: "नियमित रिपोर्ट प्राप्त करें और अपने ग्राहकों को निरंतर बढ़ते हुए देखें।",

  // Testimonials
  testi_1_text: "मार्केटिंगसेतु ने मुझे अपने फैशन डिजाइनों को पेशेवर रूप से प्रदर्शित करने और अधिक ग्राहकों से जुड़ने में मदद की है। डिजिटल उपस्थिति और प्रचार उपकरणों ने डिजाइनर सानू को बढ़ाना बहुत आसान बना दिया है।",
  testi_1_role: "मालिक, डिजाइनर सानू",
  testi_2_text: "मार्केटिंगसेतु से मेरे ब्यूटी पार्लर को प्रमोट करना और ग्राहकों से जुड़े रहना बहुत आसान हो गया है। हमारी ऑनलाइन दृश्यता में सुधार हुआ है, और हमें स्थानीय ग्राहकों से अधिक पूछताछ मिल रही है।",
  testi_2_role: "मालिक, अमृता ब्यूटी पार्लर",
  testi_3_text: "मार्केटिंगसेतु ने हमारे नवीनतम कलेक्शन को बढ़ावा देना बहुत आसान बना दिया है। ग्राहक आसानी से हमारे ऑफ़र और उत्पादों की खोज कर सकते हैं, और हमें अपने स्थानीय दर्शकों से बेहतर जुड़ाव मिल रहा है।",
  testi_3_role: "मालिक, वीआईपी मेन्स वेयर",

  // About Values
  about_val_1_title: "व्यावहारिक सलाह",
  about_val_1_desc: "हम वही सलाह देते हैं जो आपके व्यवसाय के लिए उपयुक्त हो, न कि कोई मुश्किल टूल्स।",
  about_val_2_title: "सरल भाषा में सपोर्ट",
  about_val_2_desc: "हर क्लाइंट को व्हाट्सएप पर उनकी सुविधानुसार सरल भाषा में सपोर्ट मिलता है।",
  about_val_3_title: "सच्ची ग्रोथ रिपोर्ट",
  about_val_3_desc: "मंथली लीड्स और मैसेजेस की स्पष्ट रिपोर्ट — कोई दिखावा नहीं।",

  // About Steps
  about_step_1_title: "आपके बिज़नेस को समझना",
  about_step_1_desc: "आपके प्रोडक्ट्स और कस्टमर्स को समझने के लिए एक छोटी व्हाट्सएप कॉल या चैट।",
  about_step_2_title: "सेतु का निर्माण",
  about_step_2_desc: "हम कुछ ही दिनों में आपके व्हाट्सएप ऑटोमेशन, लैंडिंग पेज और गूगल सेटअप करते हैं।",
  about_step_3_title: "सपोर्ट और ग्रोथ",
  about_step_3_desc: "लगातार सपोर्ट, ऑफर्स के आइडियाज और सरल रिपोर्ट ताकि आपको पूरा भरोसा रहे।",

  // About Page Serve tags
  about_tag_retail: "रिटेल और बुटीक",
  about_tag_restaurant: "रेस्टोरेंट और कैटरिंग",
  about_tag_real_estate: "रियल एस्टेट",
  about_tag_services: "सर्विसेज और ट्रेडर्स",

  // Contact Info keys
  contact_info_email: "ईमेल",
  contact_info_location: "पता",
  contact_info_hours: "समय",
  contact_info_hours_val: "सोम – शनि, सुबह ९ से शाम ७ बजे",
  contact_info_wa_now: "💬 तुरंत व्हाट्सएप करें",
  contact_form_plan_select: "प्लान चुनें",
  contact_form_plan_not_sure: "अभी तय नहीं",
  contact_form_msg_placeholder: "हम किस तरह आपकी सहायता कर सकते हैं...",
  contact_form_btn: "व्हाट्सएप पर भेजें 💬",

  // Faqs lists
  faq_pricing_q1: "क्या कोई हिडन चार्ज है?",
  faq_pricing_a1: "बिल्कुल नहीं। हम साल में केवल एक बार निश्चित शुल्क लेते हैं। ऑनबोर्डिंग या सेटअप के लिए कोई अतिरिक्त फीस नहीं ली जाती।",
  faq_pricing_q2: "क्या मैं अपना प्लान कभी भी बदल सकता हूँ?",
  faq_pricing_a2: "हाँ, आप किसी भी समय अपने प्लान को अपग्रेड या डाउनग्रेड कर सकते हैं। बची हुई राशि तदनुसार एडजस्ट की जाएगी।",
  faq_pricing_q3: "क्या इसमें व्हाट्सएप एपीआई की लागत शामिल है?",
  faq_pricing_a3: "हमारे प्लान्स सामान्य उपयोग के लिए पर्याप्त हैं। बहुत बड़े पैमाने पर संदेश भेजने की स्थिति में मामूली व्हाट्सएप चार्जेस की जानकारी आपको पहले ही दी जाएगी।",

  faq_contact_q1: "साइन अप के बाद काम शुरू होने में कितना समय लगता है?",
  faq_contact_a1: "आम तौर पर ५ से ७ कार्यदिवसों में आपका लैंडिंग पेज और व्हाट्सएप सिस्टम तैयार हो जाता है।",
  faq_contact_q2: "क्या मुझे टेक्निकल नॉलेज होना जरूरी है?",
  faq_contact_a2: "बिल्कुल नहीं। हमारी सपोर्ट टीम पूरा सेटअप, डिज़ाइन और टेक्निकल काम खुद संभालती है।",
  faq_contact_q3: "क्या मैं अपना मौजूदा मोबाइल नंबर इस्तेमाल कर सकता हूँ?",
  faq_contact_a3: "हाँ, आप अपने वर्तमान बिजनेस नंबर पर भी व्हाट्सएप ऑटोमेशन एक्टिवेट कर सकते हैं।",

  // Compare Table translation elements
  comp_feat_lp: "लैंडिंग पेजेस",
  comp_feat_wa: "व्हाट्सएप ब्रॉडकास्ट",
  comp_feat_mc: "मिस्ड कॉल ऑटो एसएमएस",
  comp_feat_gb: "गूगल बिजनेस सेटअप",
  comp_feat_domain: "कस्टम डोमेन",
  comp_feat_reports: "मासिक रिपोर्ट्स",
  comp_feat_call: "ऑनबोर्डिंग कॉल",
  comp_feat_support: "सपोर्ट",
  comp_feat_price: "कीमत / वर्ष",
  comp_val_month: "१ / महीना",
  comp_val_unlimited: "असीमित",
  error_name_required: "नाम आवश्यक है",
  error_phone_required: "फ़ोन नंबर आवश्यक है",
  error_phone_invalid: "कृपया १० अंकों का वैध फ़ोन नंबर दर्ज करें",
  error_business_required: "व्यवसाय का नाम आवश्यक है",
  error_plan_required: "कृपया एक प्लान चुनें",
  error_message_required: "संदेश आवश्यक है",

  // Fallbacks synced from English
  contact_faq_eyebrow: "Quick Answers",
  contact_faq_heading: "Before you reach out",
  reviews_stat_rating: "Average Rating",
  reviews_stat_reviews: "Client Reviews",
  reviews_stat_served: "Businesses Served",
  plan_quick_badge: "🌿 Starter · Setu Connect",
  plan_smart_badge: "🚀 Growth · Setu Bridge",
  plan_power_badge: "👑 Pro · Setu Summit",
  plan_feat_digital_card: "Digital Business Card",
  plan_feat_festivals: "Festival Social Media Posts",
  plan_feat_everything_starter: "Everything in Starter",
  plan_feat_everything_growth: "Everything in Growth",
  plan_feat_domain: "Custom Landing Page + Domain",
  error_form_invalid: "Please correct the errors in the form.",
  // Business Page UI
  bp_products_heading: "उत्पाद और सेवाएं",
  bp_see_all: "सभी देखें",
  bp_view_all: "सभी देखें",
  bp_more_products: "और उत्पाद",
  bp_testimonials_heading: "हमारे खुश ग्राहक क्या कहते हैं",
  bp_testimonials_subtext: "हमारे सदस्यों की ईमानदार समीक्षाएं",
  bp_enquiry_heading: "जांच भेजें",
  bp_full_name: "पूरा नाम",
  bp_name_placeholder: "अपना नाम दर्ज करें",
  bp_mobile_number: "मोबाइल नंबर",
  bp_mobile_placeholder: "मोबाइल नंबर",
  bp_message: "संदेश *",
  bp_message_placeholder: "अपना संदेश दर्ज करें",
  bp_reset: "रीसेट करें",
  bp_submit: "जमा करें",
  bp_submitting: "जमा हो रहा है...",
  bp_enquiry_success: "जांच सफलतापूर्वक जमा हुई! हम जल्द ही आपसे संपर्क करेंगे।",
  bp_message_btn: "संदेश",
  bp_call_btn: "अभी कॉल करें",
  bp_read_more: "और पढ़ें",
  bp_read_less: "कम दिखाएं",
  bp_product_enquiry_title: "उत्पाद जांच",
  bp_product_enquiry_subtitle: "अपना विवरण भरें और हम आपसे वापस संपर्क करेंगे",
  bp_select_option: "विकल्प चुनें *",
  bp_send_enquiry: "जांच भेजें",
  bp_enquiry_sent: "✓ जांच भेजी गई!",
  bp_full_name_modal: "पूरा नाम *",
  bp_mobile_modal: "मोबाइल नंबर *",
  bp_mobile_modal_placeholder: "अपना मोबाइल नंबर दर्ज करें",
  bp_message_modal: "संदेश *",
  bp_message_modal_placeholder: "कोई विशेष प्रश्न या आवश्यकताएं...",
  bp_created_with: "के साथ बनाया गया",
  bp_get_page: "अपना व्यवसाय पृष्ठ प्राप्त करें",
  bp_location_heading: "हमारा स्थान",
  bp_location_subtext: "दिशा-निर्देश और विजिटिंग आवर्स के लिए नीचे दिए गए मानचित्र पर हमें खोजें।",
  bp_view_on_maps: "Google Maps पर देखें",
  bp_owner_about: "परिचय",
  bp_owner_happy_customers: "संतुष्ट ग्राहक",
  bp_owner_years_exp: "वर्षों का अनुभव",
  bp_social_links: "सोशल लिंक्स",
  bp_action_call: "कॉल करें",
  bp_action_whatsapp: "WhatsApp",
  bp_action_email: "ईमेल",
  bp_action_location: "स्थान",
  bp_gallery_heading: "गैलरी",
  bp_more: "और",

  // Showcase Grid Localization
  showcase_badge_main: "मुख्य उत्पाद",
  showcase_badge_growth: "ग्रोथ ऐड-ऑन",
  showcase_header_eyebrow: "CORE PRODUCTS & ADD-ONS",
  showcase_header_title: "डिजिटल बिजनेस कार्ड्स और कस्टम लैंडिंग पेजेस",
  showcase_header_subtext: "स्वचालित व्हाट्सएप मैसेजिंग और ग्राहक वृद्धि टूल से लैस हमारे डिजिटल पहचान समाधान।",

  showcase_filter_all: "सभी उत्पाद",
  showcase_filter_main: "मुख्य उत्पाद",
  showcase_filter_growth: "ग्रोथ ऐड-ऑन्स",

  showcase_card_digital_title: "स्मार्ट डिजिटल विजिटिंग कार्ड",
  showcase_card_digital_desc: "आपका व्यावसायिक डिजिटल विजिटिंग कार्ड — व्हाट्सएप, QR कोड, इंस्टाग्राम और गूगल से एक टैप में शेयर करें। अपनी सेवाएं, फोटो, संपर्क विवरण और सीधी पूछताछ बटन दिखाएं। 15 मिनट में लाइव।",
  showcase_bullet_digital_1: "संपर्क और प्रोफाइल लिंक्स",
  showcase_bullet_digital_2: "व्हाट्सएप टैप-टू-कनेक्ट",
  showcase_bullet_digital_3: "तुरंत शेयर करने योग्य QR लिंक",
  showcase_btn_digital: "डिजिटल कार्ड प्राप्त करें",

  showcase_card_wa_title: "व्हाट्सएप मार्केटिंग",
  showcase_card_wa_desc: "अपने ग्राहक जहां सबसे ज्यादा समय बिताते हैं वहां सीधे पहुंचकर अपनी बिक्री बढ़ाएं।",
  showcase_bullet_wa_1: "ऑटोमेटेड पूछताछ रिप्लाई",
  showcase_bullet_wa_2: "लीड कैप्चर सिस्टम",
  showcase_bullet_wa_3: "अभियान ब्रॉडकास्ट इंजन",
  showcase_btn_wa: "व्हाट्सएप ऑटोमेशन प्राप्त करें",

  showcase_card_lp_title: "लैंडिंग पेज डिज़ाइन",
  showcase_card_lp_desc: "आपके व्यवसाय के लिए अधिक ग्राहक लाने वाला तेज़ और आकर्षक मोबाइल-फ्रेंडली लैंडिंग पेज।",
  showcase_bullet_lp_1: "1 कस्टम लैंडिंग पेज",
  showcase_bullet_lp_2: "कस्टम डोमेन कनेक्शन",
  showcase_bullet_lp_3: "मोबाइल और SEO ऑप्टिमाइज्ड",
  showcase_btn_lp: "लैंडिंग पेजेस देखें",

  showcase_card_gb_title: "गूगल बिजनेस सेटअप",
  showcase_card_gb_desc: "अपनी गूगल मैप्स प्रोफाइल को टॉप रैंकिंग पर लाएं ताकि स्थानीय ग्राहक आपको आसानी से ढूंढ सकें।",
  showcase_bullet_gb_1: "गूगल मैप्स रैंकिंग",
  showcase_bullet_gb_2: "प्रोफाइल ऑप्टिमाइज़ेशन",
  showcase_bullet_gb_3: "ग्राहक रिव्यू सेटअप",
  showcase_btn_gb: "गूगल सेटअप देखें",

  showcase_card_ie_title: "आय और व्यय ट्रैकर",
  showcase_card_ie_desc: "अपने व्यवसाय की दैनिक आय और खर्चों को आसानी से ट्रैक करें और सटीक लाभ-हानि जानें।",
  showcase_bullet_ie_1: "दैनिक लेजर शीट्स",
  showcase_bullet_ie_2: "कैश फ्लो रिपोर्ट्स",
  showcase_bullet_ie_3: "लाभप्रदता एनालिटिक्स",
  showcase_btn_ie: "ट्रैकर विवरण देखें",

  showcase_card_pc_title: "डिजिटल उत्पाद कैटलॉग",
  showcase_card_pc_desc: "अपने सभी उत्पादों और सेवाओं को एक आकर्षक पेज पर प्रदर्शित करें जहां से ग्राहक सीधे पूछताछ कर सकें।",
  showcase_bullet_pc_1: "असीमित वस्तुओं का प्रदर्शन",
  showcase_bullet_pc_2: "डायरेक्ट व्हाट्सएप ऑर्डर्स",
  showcase_bullet_pc_3: "फोटो गैलरी और कीमतें",
  showcase_btn_pc: "कैटलॉग विवरण देखें",

  // Service Block Page Keys
  service_block_card_title: "स्मार्ट डिजिटल विजिटिंग कार्ड",
  service_block_card_lead: "आपका व्यावसायिक डिजिटल विजिटिंग कार्ड — व्हाट्सएप, क्यूआर कोड, इंस्टाग्राम और गूगल से एक टैप में शेयर करें। अपनी सेवाएं, फोटो, संपर्क विवरण और सीधी पूछताछ बटन दिखाएं। 15 मिनट में लाइव।",
  service_block_wa_title: "व्हाट्सएप ऑटो मैसेजिंग",
  service_block_wa_lead: "फिर कभी कोई ग्राहक न खोएं। हमारा व्हाट्सएप ऑटो मैसेजिंग ऐड-ऑन हर पूछताछ और मिस-कॉल का 24/7 स्वचालित जवाब देता है — यहां तक कि जब आप सो रहे हों।",
  service_block_log_title: "आय और व्यय का हिसाब (खाता)",
  service_block_log_lead: "अपनी हर बिक्री, भुगतान और खर्च को सीधे अपने फोन से ट्रैक करें। छोटे व्यापारियों के लिए सरल दैनिक खाता बही — सीए या जटिल सॉफ्टवेयर की जरूरत नहीं।",
  service_block_catalog_title: "उत्पाद और सेवाएं कैटलॉग",
  service_block_catalog_lead: "अपने डिजिटल कार्ड पर फोटो, विवरण, कीमतों और डायरेक्ट व्हाट्सएप पूछताछ बटन के साथ अपने सभी उत्पाद दिखाएं — बिना किसी ऐप या वेबसाइट के आपकी ऑनलाइन दुकान।",
  service_block_gallery_title: "फोटो गैलरी शोकेस",
  service_block_gallery_lead: "ग्राहकों का विश्वास तुरंत हासिल करें। अपनी दुकान, काम, भोजन या प्रोजेक्ट्स की तस्वीरें सीधे अपने डिजिटल कार्ड पर अपलोड करें।",
  service_block_reviews_title: "ग्राहक समीक्षाएं और प्रतिक्रिया",
  service_block_reviews_lead: "संतुष्ट ग्राहकों को अपने लिए बिक्री करने दें। नए आगंतुकों को खरीदार बनाने के लिए अपने डिजिटल कार्ड पर प्रामाणिक समीक्षाएं और रेटिंग एकत्र करें और दिखाएं।",

  // Hostinger Pricing Section Keys
  home_pricing_subheading: "अपने व्यवसाय के लिए सही डिजिटल ग्रोथ प्लान चुनें — शून्य सेटअप शुल्क के साथ कभी भी अपग्रेड या बदलें।",
  pricing_billing_yearly: "वार्षिक प्लान",
  pricing_billing_monthly: "मासिक प्लान",
  pricing_save_badge: "50% बचत",
  pricing_whats_included: "क्या शामिल है:",
  pricing_billed_annually: "वार्षिक बिलिंग • शून्य सेटअप शुल्क",
  pricing_choose_plan: "प्लान चुनें",

  // Plan 1 Feature Keys
  plan_p1_f1_title: "डिजिटल बिजनेस कार्ड",
  plan_p1_f1_desc: "मिस्ड कॉल या पूछताछ पर तुरंत स्वचालित जवाब और लीड कैप्चर।",
  plan_p1_f2_title: "मिस्ड कॉल ऑटो एसएमएस",
  plan_p1_f2_desc: "मानक मोबाइल-रिस्पॉन्सिव डिजिटल बिजनेस कार्ड टेम्पलेट।",
  plan_p1_f3_title: "ब्रांड और संपर्क विवरण",
  plan_p1_f3_desc: "लोगो, व्यवसाय का नाम, श्रेणी, फोन, ईमेल और मैप लोकेशन।",
  plan_p1_f4_title: "सोशल और ओनर लिंक्स",
  plan_p1_f4_desc: "व्हाट्सएप और सोशल प्रोफाइल के लिए डायरेक्ट टैप-टू-कनेक्ट लिंक्स।",

  // Plan 2 Feature Keys
  plan_p2_f1_title: "स्टार्टर की सभी सुविधाएं",
  plan_p2_f1_desc: "",
  plan_p2_f2_title: "व्हाट्सएप ब्रॉडकास्ट",
  plan_p2_f2_desc: "आपके व्यवसाय के लिए अनुकूलित लेआउट (कैफे, ब्यूटी, सोलर, ब्रोकर, आदि)।",
  plan_p2_f3_title: "त्योहारों के सोशल मीडिया पोस्ट्स",
  plan_p2_f3_desc: "अपने उत्पाद, कीमतें, फोटो और सीधी व्हाट्सएप पूछताछ प्रदर्शित करें।",
  plan_p2_f4_title: "फोटो गैलरी और रिव्यू",
  plan_p2_f4_desc: "उच्च-रिज़ॉल्यूशन शोकेस गैलरी और ग्राहक रेटिंग प्रशंसापत्र।",

  // Plan 3 Feature Keys
  plan_p3_f1_title: "ग्रोथ की सभी सुविधाएं",
  plan_p3_f1_desc: "",
  plan_p3_f2_title: "१ लैंडिंग पेज",
  plan_p3_f2_desc: "विशेष रूप से आपके व्यावसायिक ब्रांड के लिए तैयार किया गया UI डिज़ाइन।",
  plan_p3_f3_title: "कस्टम लैंडिंग पेज + डोमेन",
  plan_p3_f3_desc: "आपके अपने .com या .in डोमेन से जुड़ा हुआ।",
  plan_p3_f4_title: "गूगल मैप्स सेटअप",
  plan_p3_f4_desc: "स्थानीय खोज मैप अनुकूलन और SEO इंडेक्स सेटअप।",

  // Carousel Demo Card Category & Titles
  demo_cat_default: "डिफ़ॉल्ट",
  demo_title_default: "डिफ़ॉल्ट",
  demo_cat_cafe: "कैफे",
  demo_title_cafe: "कैफे",
  demo_cat_solar: "सोलर",
  demo_title_solar: "सोलर",
  demo_cat_parlour: "पार्लर",
  demo_title_parlour: "पार्लर",
  demo_cat_two_wheeler: "टू व्हीलर ब्रोकर",
  demo_title_two_wheeler: "टू व्हीलर ब्रोकर",
  demo_cat_portfolio: "पर्सनल पोर्टफोलियो",
  demo_title_portfolio: "पर्सनल पोर्टफोलियो",

  // Instant Live Card Builder Simulator
  generator_badge: "त्वरित लाइव पूर्वावलोकन • सटीक बिज़नेस पेज UI",
  generator_title: "अपना डिजिटल बिज़नेस पेज डिज़ाइन करें",
  generator_title_highlight: "३० सेकंड में",
  generator_subtitle: "नीचे अपने व्यवसाय का विवरण भरें और तुरंत अपना लाइव मोबाइल-अनुकूल डिजिटल बिज़नेस पेज देखें।",
  generator_template_label: "⚡ उद्योग टेम्पलेट चुनें (Industry Template)",
  generator_tpl_cafe: "कैफे",
  generator_tpl_beauty: "ब्यूटी पार्लर",
  generator_tpl_solar: "सोलर और ऊर्जा समाधान",
  generator_tpl_broker: "टू व्हीलर ब्रोकर",
  generator_tpl_retail: "रिटेल / दुकान",
  generator_business_name_label: "व्यवसाय का नाम",
  generator_business_name_placeholder: "उदा. आर्टिसनल हर्थ कैफे",
  generator_category_chip_label: "कैटेगरी चिप",
  generator_category_chip_placeholder: "उदा. कैफे और बेकरी",
  generator_tagline_label: "टैगलाइन / स्लोगन",
  generator_tagline_placeholder: "उदा. ताजा कॉफी और स्वादिष्ट बेकरी उत्पाद",
  generator_owner_name_label: "मालिक / संस्थापक का नाम",
  generator_owner_name_placeholder: "उदा. कबीर मेहता",
  generator_owner_role_label: "पद / पदनाम",
  generator_owner_role_placeholder: "उदा. हेड बरिस्ता और मालिक",
  generator_phone_label: "फोन / व्हाट्सएप",
  generator_email_label: "ईमेल",
  generator_city_label: "शहर / स्थान",
  generator_city_placeholder: "उदा. पुणे, महाराष्ट्र",
  generator_theme_color_label: "🎨 बिज़नेस पेज थीम रंग",
  generator_publish_btn: "यह पेज मार्केटिंगसेतु के साथ लाइव करें",
  generator_footer_trust: "✓ १५ मिनट में ऑनबोर्डिंग • कस्टम डोमेन / सबडोमेन • व्हाट्सएप ऑटो-रिप्लाई इंजन",
};
