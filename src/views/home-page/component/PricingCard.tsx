...
export default function PricingCard({
  ...
  return (
    <div className={`group p-8 rounded-[24px] transition-all duration-300 relative flex flex-col justify-between ${
      isDarkCard 
      ...
      <div>
        <h3 className={`text-2xl font-bold mb-2 ${isDarkCard ? "text-white" : "text-primary"}`}>
          {plan.name}
        </h3>
        <p className={`text-sm mb-6 ${isDarkCard ? "text-white/70" : "text-secondary"}`}>
          {plan.description}
        </p>
        <div className="mb-6 flex items-baseline gap-1">
          <span className={`text-5xl font-extrabold tracking-tight ${isDarkCard ? "text-white" : "text-primary"}`}>
            {plan.price}
          </span>
        ...
        <ul className="space-y-4 mb-8">
          {displayFeatures.map((feature, index) => (
            <li key={index} className="flex items-start gap-3 text-sm">
              <span className={`font-bold flex-shrink-0 mt-0.5 ${isDarkCard ? "text-[#10C85A]" : "text-brand-main"}`}>✓</span>
              <span className={isDarkCard ? "text-white/90" : "text-secondary"}>
                <strong className={`font-semibold ${isDarkCard ? "text-white" : "text-primary dark:text-white"}`}>{feature.bold}</strong>
                {!isCompact && feature.desc && (
                  <span className={`block text-xs mt-1 ${isDarkCard ? "text-white/60" : "text-disabled"}`}>
                    {feature.desc}
                  </span>
                )}
              </span>
